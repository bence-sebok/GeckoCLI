...
void RETARGET_IRQ_NAME(void)
{
	if (RETARGET_UART->STATUS & USART_STATUS_RXDATAV)
	{
		...
		// Reads char from RX buffer and echo to TX pin
		UART_rxChar = RETARGET_RX(RETARGET_UART);
		RETARGET_TX(RETARGET_UART, UART_rxChar);
		// Unblocks the UART RX Handler task
